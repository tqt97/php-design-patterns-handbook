...
**Activity** là capability chuyên biệt của **crm platform**. Module chỉ sở hữu quyết định và dữ liệu cần thiết cho activity; nó không được biến thành service tổng hợp cho toàn platform. Input/output phải dùng ngôn ngữ nghiệp vụ, còn database, broker và provider được đặt sau port/adapter rõ ràng.

## Invariant cần bảo vệ

- **Activity duy trì trạng thái hợp lệ và ownership rõ trong crm platform.**
...
Activity timeline dùng append-only event model với source, actor, occurredAt và visibility. Projection gom email/call/note nhưng không làm mất raw provenance; backfill phải idempotent.

```mermaid
sequenceDiagram
    participant A as CRMAdapter
    participant B as ActivityIngestor
    participant C as ActivityStore
    participant D as TimelineProjection
    A->>B: receive source event
    B->>C: normalize identity/time
    C->>D: append activity
    C->>D: update timeline projection
    C->>D: serve ordered view
```


## Failure modes riêng của module

- duplicate activity; stale state; dependency timeout ở activity.
...
1. Unit test invariant: **Activity giữ trạng thái hợp lệ theo rule của crm-platform**.
...
Theo dõi **activity success rate, error rate, oldest pending age**. Log/trace tối thiểu gồm resource ID, tenant, correlation/idempotency key, command/event type, version và provider nếu có.

Dashboard của **activity** trong crm platform phải hiển thị phạm vi ảnh hưởng, giá trị nghiệp vụ liên quan, tuổi của item lâu nhất và xu hướng backlog. Alert ưu tiên breach của invariant hoặc SLA riêng của activity; exception count chỉ là tín hiệu chẩn đoán phụ.
...
3. Cô lập activity, xác minh source of truth, replay command có idempotency key và kiểm tra kết quả.
...
- Transaction boundary có đúng với invariant “Activity giữ trạng thái hợp lệ theo rule của crm-platform” không?
...
- Metric **activity error rate; pending age; business impact** có phát hiện sai lệch trước khi khách hàng báo không?
- Runbook có thao tác an toàn, idempotent và có verification query không?

## Phạm vi tài liệu

Tài liệu này tập trung riêng vào **Activity** trong `production/crm-platform/modules/activity.md`: ownership, invariant, failure recovery và evidence vận hành. Overview của platform mô tả quan hệ giữa các capability; bài này là contract review cho module và không thay thế runbook triển khai cụ thể của môi trường.
