...
**Concurrency** là capability chuyên biệt của **inventory platform**. Module chỉ sở hữu quyết định và dữ liệu cần thiết cho concurrency; nó không được biến thành service tổng hợp cho toàn platform. Input/output phải dùng ngôn ngữ nghiệp vụ, còn database, broker và provider được đặt sau port/adapter rõ ràng.

## Invariant cần bảo vệ

- **Mỗi write kiểm tra version hoặc lock scope đúng aggregate.**
...
Chọn optimistic locking cho SKU contention thấp và atomic counter/row lock cho hot SKU. Mọi retry phải re-read invariant và dùng operation id để không nhân đôi adjustment.
...
sequenceDiagram
    ...
    participant D as ConflictRetry
    A->>B: reserve quantity
    B->>C: load version/balance
    C->>D: conditional write
    C->>D: on conflict reload
    C->>D: commit once by operation id
```


## Failure modes riêng của module

- lost update; deadlock; stale retry.
...
1. Unit test invariant: **Concurrency giữ trạng thái hợp lệ theo rule của inventory-platform**.
...
Theo dõi **conflict rate, deadlock rate, retry exhaustion**. Log/trace tối thiểu gồm resource ID, tenant, correlation/idempotency key, command/event type, version và provider nếu có.

Dashboard của **concurrency** trong inventory platform phải hiển thị phạm vi ảnh hưởng, giá trị nghiệp vụ liên quan, tuổi của item lâu nhất và xu hướng backlog. Alert ưu tiên breach của invariant hoặc SLA riêng của concurrency; exception count chỉ là tín hiệu chẩn đoán phụ.
...
3. Xác định contention key, dừng retry nóng, dùng backoff, reconcile version/source of truth.
...
- Transaction boundary có đúng với invariant “Concurrency giữ trạng thái hợp lệ theo rule của inventory-platform” không?
...
- Metric **concurrency error rate; pending age; business impact** có phát hiện sai lệch trước khi khách hàng báo không?
- Runbook có thao tác an toàn, idempotent và có verification query không?

## Phạm vi tài liệu

Tài liệu này tập trung riêng vào **Concurrency** trong `production/inventory-platform/modules/concurrency.md`: ownership, invariant, failure recovery và evidence vận hành. Overview của platform mô tả quan hệ giữa các capability; bài này là contract review cho module và không thay thế runbook triển khai cụ thể của môi trường.
