...
**Warehouse Routing** là capability chuyên biệt của **inventory platform**. Module chỉ sở hữu quyết định và dữ liệu cần thiết cho warehouse routing; nó không được biến thành service tổng hợp cho toàn platform. Input/output phải dùng ngôn ngữ nghiệp vụ, còn database, broker và provider được đặt sau port/adapter rõ ràng.

## Invariant cần bảo vệ

- **Warehouse routing duy trì trạng thái hợp lệ và ownership rõ trong inventory platform.**
...
Routing policy chọn warehouse dựa trên stock, distance, SLA, split cost và capability. Allocation result phải pin decision inputs/version; fallback không được âm thầm vi phạm inventory invariant.

```mermaid
sequenceDiagram
    participant A as FulfillmentService
    participant B as RoutingPolicy
    participant C as InventoryReadModel
    participant D as WarehouseAdapters
    A->>B: build fulfillment request
    B->>C: load candidate capabilities
    C->>D: score route options
    C->>D: reserve selected stock
    C->>D: persist routing decision
```


## Failure modes riêng của module

- duplicate warehouse routing; stale state; dependency timeout ở warehouse routing.
...
1. Unit test invariant: **Warehouse Routing giữ trạng thái hợp lệ theo rule của inventory-platform**.
...
Theo dõi **warehouse routing success rate, error rate, oldest pending age**. Log/trace tối thiểu gồm resource ID, tenant, correlation/idempotency key, command/event type, version và provider nếu có.

Dashboard của **warehouse routing** trong inventory platform phải hiển thị phạm vi ảnh hưởng, giá trị nghiệp vụ liên quan, tuổi của item lâu nhất và xu hướng backlog. Alert ưu tiên breach của invariant hoặc SLA riêng của warehouse routing; exception count chỉ là tín hiệu chẩn đoán phụ.
...
3. Cô lập warehouse routing, xác minh source of truth, replay command có idempotency key và kiểm tra kết quả.
...
- Transaction boundary có đúng với invariant “Warehouse Routing giữ trạng thái hợp lệ theo rule của inventory-platform” không?
...
- Metric **warehouse-routing error rate; pending age; business impact** có phát hiện sai lệch trước khi khách hàng báo không?
- Runbook có thao tác an toàn, idempotent và có verification query không?

## Phạm vi tài liệu

Tài liệu này tập trung riêng vào **Warehouse Routing** trong `production/inventory-platform/modules/warehouse-routing.md`: ownership, invariant, failure recovery và evidence vận hành. Overview của platform mô tả quan hệ giữa các capability; bài này là contract review cho module và không thay thế runbook triển khai cụ thể của môi trường.
