...
**Cycle Count** là capability chuyên biệt của **inventory platform**. Module chỉ sở hữu quyết định và dữ liệu cần thiết cho cycle count; nó không được biến thành service tổng hợp cho toàn platform. Input/output phải dùng ngôn ngữ nghiệp vụ, còn database, broker và provider được đặt sau port/adapter rõ ràng.

## Invariant cần bảo vệ

- **Cycle count duy trì trạng thái hợp lệ và ownership rõ trong inventory platform.**
...
Cycle count tách observed quantity khỏi system quantity. Adjustment chỉ được post sau review theo threshold; luôn ghi reason, counter, approver và reference để reconciliation/audit.

```mermaid
sequenceDiagram
    participant A as Counter
    participant B as CountSession
    participant C as VariancePolicy
    participant D as Approver
    participant E as StockLedger
    A->>B: open count session
    B->>C: record blind count
    C->>D: calculate variance
    D->>E: approve if required
    D->>E: post adjustment entry
```


## Failure modes riêng của module

- duplicate cycle count; stale state; dependency timeout ở cycle count.
...
1. Contract test `CycleCountService` xác nhận mỗi adjustment tham chiếu count session, location, actor và reason code; không update/xóa ledger line cũ.
...
Theo dõi **cycle count success rate, error rate, oldest pending age**. Log/trace tối thiểu gồm resource ID, tenant, correlation/idempotency key, command/event type, version và provider nếu có.

Dashboard của **cycle count** trong inventory platform phải hiển thị phạm vi ảnh hưởng, giá trị nghiệp vụ liên quan, tuổi của item lâu nhất và xu hướng backlog. Alert ưu tiên breach của invariant hoặc SLA riêng của cycle count; exception count chỉ là tín hiệu chẩn đoán phụ.
...
3. Cô lập cycle count, xác minh source of truth, replay command có idempotency key và kiểm tra kết quả.
...
- Cycle-count session được khóa/snapshot ở thời điểm nào, và adjustment có thể bị ghi hai lần khi mobile client retry không?
...
Tài liệu này tập trung riêng vào **Cycle Count** trong `production/inventory-platform/modules/cycle-count.md`: ownership, invariant, failure recovery và evidence vận hành. Overview của platform mô tả quan hệ giữa các capability; bài này là contract review cho module và không thay thế runbook triển khai cụ thể của môi trường.
