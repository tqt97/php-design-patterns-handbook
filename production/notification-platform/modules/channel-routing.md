...
**Channel Routing** là capability chuyên biệt của **notification platform**. Module chỉ sở hữu quyết định và dữ liệu cần thiết cho channel routing; nó không được biến thành service tổng hợp cho toàn platform. Input/output phải dùng ngôn ngữ nghiệp vụ, còn database, broker và provider được đặt sau port/adapter rõ ràng.

## Invariant cần bảo vệ

- **Mỗi notification chọn channel theo preference, capability và fallback policy.**
...
Channel routing dùng policy trên preference, consent, urgency, cost và channel health. Quyết định phải lưu để retry không đổi kênh ngoài ý muốn; fallback chỉ áp dụng khi business rule cho phép.

```mermaid
sequenceDiagram
    participant A as NotificationService
    participant B as RoutingPolicy
    participant C as PreferenceStore
    participant D as ChannelAdapter
    A->>B: build delivery intent
    B->>C: evaluate allowed channels
    C->>D: select stable route
    C->>D: send through adapter
    C->>D: record outcome
```
...
- unsupported channel; provider outage; duplicate fallback.
...
1. Unit test invariant: **Channel Routing giữ trạng thái hợp lệ theo rule của notification-platform**.
...
Theo dõi **delivery rate by channel, fallback rate, age**. Log/trace tối thiểu gồm resource ID, tenant, correlation/idempotency key, command/event type, version và provider nếu có.

Dashboard của **channel routing** trong notification platform phải hiển thị phạm vi ảnh hưởng, giá trị nghiệp vụ liên quan, tuổi của item lâu nhất và xu hướng backlog. Alert ưu tiên breach của invariant hoặc SLA riêng của channel routing; exception count chỉ là tín hiệu chẩn đoán phụ.
...
3. Disable provider route, replay undelivered message bằng message id, không gửi lại delivered channel.
...
- Transaction boundary có đúng với invariant “Channel Routing giữ trạng thái hợp lệ theo rule của notification-platform” không?
...
- Metric **channel-routing error rate; pending age; business impact** có phát hiện sai lệch trước khi khách hàng báo không?
- Runbook có thao tác an toàn, idempotent và có verification query không?

## Phạm vi tài liệu

Tài liệu này tập trung riêng vào **Channel Routing** trong `production/notification-platform/modules/channel-routing.md`: ownership, invariant, failure recovery và evidence vận hành. Overview của platform mô tả quan hệ giữa các capability; bài này là contract review cho module và không thay thế runbook triển khai cụ thể của môi trường.
