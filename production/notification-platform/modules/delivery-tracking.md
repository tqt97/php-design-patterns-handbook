...
**Delivery Tracking** là capability chuyên biệt của **notification platform**. Module chỉ sở hữu quyết định và dữ liệu cần thiết cho delivery tracking; nó không được biến thành service tổng hợp cho toàn platform. Input/output phải dùng ngôn ngữ nghiệp vụ, còn database, broker và provider được đặt sau port/adapter rõ ràng.

## Invariant cần bảo vệ

- **Delivery tracking duy trì trạng thái hợp lệ và ownership rõ trong notification platform.**
...
Mỗi delivery attempt có correlation id và lifecycle accepted/sent/delivered/failed. Provider callback được map vào canonical status nhưng giữ raw payload; status regression bị từ chối trừ khi policy cho phép.

```mermaid
sequenceDiagram
    participant A as ChannelProvider
    participant B as WebhookInbox
    participant C as DeliveryTracker
    participant D as NotificationReadModel
    A->>B: receive status callback
    B->>C: deduplicate event
    C->>D: map provider status
    C->>D: apply monotonic transition
    C->>D: update delivery view
```


## Failure modes riêng của module

- duplicate delivery tracking; stale state; dependency timeout ở delivery tracking.
...
1. Unit test invariant: **Delivery Tracking giữ trạng thái hợp lệ theo rule của notification-platform**.
...
Theo dõi **delivery tracking success rate, error rate, oldest pending age**. Log/trace tối thiểu gồm resource ID, tenant, correlation/idempotency key, command/event type, version và provider nếu có.

Dashboard của **delivery tracking** trong notification platform phải hiển thị phạm vi ảnh hưởng, giá trị nghiệp vụ liên quan, tuổi của item lâu nhất và xu hướng backlog. Alert ưu tiên breach của invariant hoặc SLA riêng của delivery tracking; exception count chỉ là tín hiệu chẩn đoán phụ.
...
3. Cô lập delivery tracking, xác minh source of truth, replay command có idempotency key và kiểm tra kết quả.
...
- Transaction boundary có đúng với invariant “Delivery Tracking giữ trạng thái hợp lệ theo rule của notification-platform” không?
...
- Metric **delivery-tracking error rate; pending age; business impact** có phát hiện sai lệch trước khi khách hàng báo không?
- Runbook có thao tác an toàn, idempotent và có verification query không?

## Phạm vi tài liệu

Tài liệu này tập trung riêng vào **Delivery Tracking** trong `production/notification-platform/modules/delivery-tracking.md`: ownership, invariant, failure recovery và evidence vận hành. Overview của platform mô tả quan hệ giữa các capability; bài này là contract review cho module và không thay thế runbook triển khai cụ thể của môi trường.
