...
**Payment** là capability chuyên biệt của **booking platform**. Module chỉ sở hữu quyết định và dữ liệu cần thiết cho payment; nó không được biến thành service tổng hợp cho toàn platform. Input/output phải dùng ngôn ngữ nghiệp vụ, còn database, broker và provider được đặt sau port/adapter rõ ràng.

## Invariant cần bảo vệ

- **Payment duy trì trạng thái hợp lệ và ownership rõ trong booking platform.**
...
Booking orchestration không gộp payment và reservation vào một transaction phân tán. Dùng saga: giữ chỗ có TTL, authorize payment, confirm booking; nếu payment lỗi hoặc timeout thì release hold/void authorization theo trạng thái quan sát được.

```mermaid
sequenceDiagram
    participant A as BookingSaga
    ...
    C->>D: confirm booking
    C->>D: on failure compensate
    C->>D: record saga state
```


## Failure modes riêng của module

- duplicate payment; stale state; dependency timeout ở payment.
...
1. Unit test invariant: **Payment giữ trạng thái hợp lệ theo rule của booking-platform**.
...
Theo dõi **payment success rate, error rate, oldest pending age**. Log/trace tối thiểu gồm resource ID, tenant, correlation/idempotency key, command/event type, version và provider nếu có.

Dashboard của **payment** trong booking platform phải hiển thị phạm vi ảnh hưởng, giá trị nghiệp vụ liên quan, tuổi của item lâu nhất và xu hướng backlog. Alert ưu tiên breach của invariant hoặc SLA riêng của payment; exception count chỉ là tín hiệu chẩn đoán phụ.
...
3. Cô lập payment, xác minh source of truth, replay command có idempotency key và kiểm tra kết quả.
...
- Transaction boundary có đúng với invariant “Payment giữ trạng thái hợp lệ theo rule của booking-platform” không?
...
- Metric **payment error rate; pending age; business impact** có phát hiện sai lệch trước khi khách hàng báo không?
- Runbook có thao tác an toàn, idempotent và có verification query không?

## Phạm vi tài liệu

Tài liệu này tập trung riêng vào **Payment** trong `production/booking-platform/modules/payment.md`: ownership, invariant, failure recovery và evidence vận hành. Overview của platform mô tả quan hệ giữa các capability; bài này là contract review cho module và không thay thế runbook triển khai cụ thể của môi trường.
