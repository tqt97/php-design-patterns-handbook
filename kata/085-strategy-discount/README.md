# Kata 85: Strategy trong Discount

## Bối cảnh và lý do chọn bài

Module **Discount** đang đánh giá coupon và promotion. Invariant bắt buộc là **tổng giảm không vượt giá trị đơn**; failure cần quan sát là **nhiều rule cùng áp dụng hoặc stacking sai**. Kata này dùng **Strategy** để luyện cách tách policy/thuật toán có cùng contract để use case thay đổi policy mà không sửa orchestration. Đây là tình huống tổng hợp phục vụ học tập, không giả định có một đề bài ẩn khác.

## Code smell cần tìm

Hãy dựng hoặc đọc starter code và đánh dấu nơi `'DiscountService'` vừa điều phối use case, vừa biết concrete detail thuộc change axis của **Strategy**. Dấu hiệu cần refactor không phải method dài đơn thuần, mà là mỗi lần thay đổi Discount đều buộc sửa cùng nhánh, cùng dependency hoặc cùng lifecycle logic.

## Mục tiêu thiết kế

Context phụ thuộc policy contract; composition root hoặc factory chọn concrete strategy. Sau refactor, client phải biết ít concrete detail hơn và invariant **tổng giảm không vượt giá trị đơn** vẫn được bảo vệ.
...
classDiagram
    ...
    class DiscountPolicy {
      <<interface>>
      +evaluate(DiscountContext) DiscountResult
    }
    class StandardDiscountPolicy
    class AlternativeDiscountPolicy
    DiscountContext --> DiscountPolicy : được đánh giá bởi
    DiscountPolicy <|.. StandardDiscountPolicy
    DiscountPolicy <|.. AlternativeDiscountPolicy
```

Sơ đồ mô tả đúng cơ chế **Strategy** trong miền **Discount**. Khi triển khai, hãy giữ invariant: **không áp dụng ưu đãi ngoài điều kiện**. Participant trong sơ đồ là vocabulary gợi ý; đổi tên được, nhưng hướng phụ thuộc và failure boundary không được đảo ngược.

## Câu hỏi review

1. Change axis của **Strategy** trong Discount có bằng chứng từ requirement hay chỉ là dự đoán?
