# Kata 147: Adapter trong Payment

## Bối cảnh và lý do chọn bài

Module **Payment** đang authorize, capture và refund qua gateway. Invariant bắt buộc là **không charge trùng khi client retry**; failure cần quan sát là **gateway decline/timeout hoặc callback đến trễ**. Kata này dùng **Adapter** để luyện cách dịch contract của SDK/hệ thống ngoài sang ngôn ngữ nội bộ tại một boundary duy nhất. Đây là tình huống tổng hợp phục vụ học tập, không giả định có một đề bài ẩn khác.

## Code smell cần tìm

Hãy dựng hoặc đọc starter code và đánh dấu nơi `'PaymentService'` vừa điều phối use case, vừa biết concrete detail thuộc change axis của **Adapter**. Dấu hiệu cần refactor không phải method dài đơn thuần, mà là mỗi lần thay đổi Payment đều buộc sửa cùng nhánh, cùng dependency hoặc cùng lifecycle logic.

## Mục tiêu thiết kế

Adapter implement target port, map dữ liệu và translate error của adaptee. Sau refactor, client phải biết ít concrete detail hơn và invariant **không charge trùng khi client retry** vẫn được bảo vệ.
...
sequenceDiagram
    ...
    participant P as PaymentPolicyPort
    participant A as LegacyPaymentPolicyAdapter
    participant S as External SDK
    U->>P: xử lý PaymentCommand
    P->>A: gọi contract nội bộ
    A->>S: map request
    S-->>A: vendor response/error
    A-->>U: PaymentResult hoặc domain error
```

Sơ đồ mô tả đúng cơ chế **Adapter** trong miền **Payment**. Khi triển khai, hãy giữ invariant: **một idempotency key không tạo hai charge**. Participant trong sơ đồ là vocabulary gợi ý; đổi tên được, nhưng hướng phụ thuộc và failure boundary không được đảo ngược.

## Câu hỏi review

1. Change axis của **Adapter** trong Payment có bằng chứng từ requirement hay chỉ là dự đoán?
...
```bash
php kata/147-adapter-payment/solution.php
```

## Tài liệu liên quan

- Bài liên quan trực tiếp: **Adapter** trong **Payment**; dùng liên kết dưới đây để đối chiếu lý thuyết và bài thực hành.
