# Kata 6: State trong Audit

## Bối cảnh và lý do chọn bài

Module **Audit** đang ghi actor, action và before/after. Invariant bắt buộc là **log không bỏ sót và không chứa secret**; failure cần quan sát là **audit sink lỗi hoặc correlation bị mất**. Kata này dùng **State** để luyện cách mô hình hóa behavior theo lifecycle và kiểm soát transition hợp lệ. Đây là tình huống tổng hợp phục vụ học tập, không giả định có một đề bài ẩn khác.

## Code smell cần tìm

Hãy dựng hoặc đọc starter code và đánh dấu nơi `'AuditService'` vừa điều phối use case, vừa biết concrete detail thuộc change axis của **State**. Dấu hiệu cần refactor không phải method dài đơn thuần, mà là mỗi lần thay đổi Audit đều buộc sửa cùng nhánh, cùng dependency hoặc cùng lifecycle logic.

## Mục tiêu thiết kế

Context ủy quyền behavior cho current state hoặc transition policy; guard chặn transition sai. Sau refactor, client phải biết ít concrete detail hơn và invariant **log không bỏ sót và không chứa secret** vẫn được bảo vệ.
...
stateDiagram-v2
    ...
    Completed --> [*]
    note right of Active: guard bảo vệ invariant của AuditEntry
```

Sơ đồ mô tả đúng cơ chế **State** trong miền **Audit**. Khi triển khai, hãy giữ invariant: **evidence bất biến và truy vết được**. Participant trong sơ đồ là vocabulary gợi ý; đổi tên được, nhưng hướng phụ thuộc và failure boundary không được đảo ngược.

## Câu hỏi review

1. Change axis của **State** trong Audit có bằng chứng từ requirement hay chỉ là dự đoán?
...
```bash
php kata/006-state-audit/solution.php
```

## Tài liệu liên quan

- Bài liên quan trực tiếp: **State** trong **Audit**; dùng liên kết dưới đây để đối chiếu lý thuyết và bài thực hành.
